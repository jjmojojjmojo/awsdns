...
Source: http://notmysock.org/blog/hacks/a-twisted-dns-story.html
"""
...
class EC2Resolver(client.Resolver):
    ...
    def parse_config(self):
        ...
        """
        print config.items('awsdns')
        
        self.aws_region = config.get('awsdns', 'aws_region')
        self.aws_access_key_id = config.get('awsdns', 'aws_access_key_id')
        self.aws_secret_access_key = config.get('awsdns', 'aws_secret_access_key')
        
        self.forward_filter = config.get('awsdns', 'forward', 'tag:Name')
        self.reverse_filter = config.get('awsdns', 'reverse', 'private_ip_address')
        
        self.extra = config.get('awsdns', 'extra', '').split()
    ...
    def filterAnswers(self, message):
        ...
        for answer in message.answers:
            pprint.pprint(answer.__dict__)
        
        for query in message.queries:
            pprint.pprint(query.__dict__)
